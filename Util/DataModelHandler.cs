...
namespace ObAuto
{
    public class DataModelHandler<T> where T : new()
    {
        #region DataTable to Model
        /// <summary>
        /// 填充对象列表：用DataSet的第一个表填充实体类列表
        /// </summary>
        /// <param name="ds">DataSet</param>
        /// <returns></returns>
        public List<T> FillModel(DataSet ds)
        {
            return FillModel(ds, 0);
        }

        /// <summary>
        /// 填充对象列表：用DataSet的第index个表填充实体类列表
        /// </summary>
        /// <param name="ds"></param>
        ...
        public DataTable FillDataTable(List<T> modelList)
        {
            ...
            foreach (T model in modelList)
            {
                ...
                ////dataRow = FillDataRow(dt, model);
                dt.Rows.Add(dataRow);
            }
            return dt;
        }

        public DataRow FillDataRow(DataTable dt, T model)
        {
            DataRow dataRow = dt.NewRow();
            foreach (PropertyInfo propertyInfo in typeof(T).GetProperties())
            {
