...
namespace ObAuto
{
    ...
    public sealed class SQLiteHelper
    {
        ...
        public static string ConnectionString
        {
            get
            {
                string dbcs = ConfigurationManager.ConnectionStrings["SQLiteConn"].ConnectionString;
                string isEncrypt = ConfigurationManager.AppSettings["IsEncrypt"];
                if (!string.IsNullOrWhiteSpace(isEncrypt) && isEncrypt == "1")
                {
                    dbcs = DesSecurity.Decrypt(dbcs);
                }
                return dbcs;
            }
        }
